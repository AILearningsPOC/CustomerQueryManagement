...
async function scrapeWithApify(url) {
  ...
  const pageFn = `async function pageFunction(context) {
    ...
    for (const sel of bbSelectors) {
      ...
      if (els.length > 0) { els.forEach(el => { const t = el.innerText.trim(); if (t.length > 5) results.push({ question_text: t, existing_answer: null, answer_status: 'unanswered', date_asked: null, customer_name: null }); }); break; }
    }
    document.querySelectorAll('[data-hook="ask-btf-question-text"]').forEach(el => { const t = el.innerText.trim(); if (t.length > 5) results.push({ question_text: t, existing_answer: null, answer_status: 'unanswered', date_asked: null, customer_name: null }); });
    if (results.length === 0) {
      document.querySelectorAll('span,p,div').forEach(el => { if (el.children.length === 0) { const t = (el.innerText||'').trim(); if (t.endsWith('?') && t.length > 15 && t.length < 400) results.push({ question_text: t, existing_answer: null, answer_status: 'unanswered', date_asked: null, customer_name: null }); } });
    }
    return [...new Map(results.map(q => [q.question_text, q])).values()].slice(0, 30);
  }`;
  console.log('[Apify] Starting web-scraper for ' + url);
  const startRes = await axios.post('https://api.apify.com/v2/acts/' + actorId + '/runs',
  ...
  return (res.data || []).filter(q => q.question_text?.length > 5);
}

// Main scrape function
async function scrapeTargetItem(target, engine) {
  const log = { target_id: target.id, retailer: target.retailer, product_name: target.product_name, url: target.url, engine_used: engine, scraped_at: new Date().toISOString(), found_count: 0, new_count: 0, error: null };
  try {
    let rawQuestions = [];
    if (engine === 'apify') {
      rawQuestions = await scrapeWithApify(target.url).catch(e => { log.error = e.message; return []; });
    } else {
      try {
      ...
      } catch (err) { log.error = err.message; console.error(`[scrapeTargetItem] ${target.retailer} failed:`, err.message); }
    }
    ...
    for (const q of rawQuestions) {
      if (!q.question_text?.trim() || q.question_text.trim().length < 5) continue;
      ...
      const { error: insertErr } = await supabase.from('questions').insert({ question_text: q.question_text.trim(), existing_answer: q.existing_answer || null, date_asked: q.date_asked || null, customer_name: q.customer_name || null, answer_status: q.answer_status || 'unanswered', retailer: target.retailer, product_name: target.product_name, product_url: target.url, content_hash: hash, status: 'pending', source: 'scraper' });
      if (insertErr) console.error('[scrapeTargetItem] Insert error:', insertErr.message);
      else newCount++;
    }
    log.new_count = newCount;
    try { await supabase.from('scrape_targets').update({ last_scraped_at: new Date().toISOString(), questions_found_total: (target.questions_found_total || 0) + newCount }).eq('id', target.id); } catch (e) {}
  } catch (err) { log.error = err.message; console.error(`[scrapeTargetItem] Top-level error:`, err.message); }
...
async function scrapeAll(engine) {
  ...
  for (const target of targets) {
    try { logs.push(await scrapeTargetItem(target, engine)); }
    catch (err) { console.error(`[scrapeAll] Failed for ${target.product_name}:`, err.message); logs.push({ product_name: target.product_name, error: err.message, new_count: 0, found_count: 0 }); }
  }
...
function extractQAFromJSON(obj, depth = 0) {
  ...
  if (Array.isArray(obj)) { for (const item of obj) results.push(...extractQAFromJSON(item, depth + 1)); }
    ...
    if (text && typeof text === 'string' && text.length > 5) results.push({ question_text: text.trim(), existing_answer: obj.answerText || obj.answers?.[0]?.answerText || null, answer_status: obj.answers?.length > 0 ? 'answered' : 'unanswered', date_asked: obj.submissionTime ? new Date(obj.submissionTime).toISOString() : null, customer_name: obj.userNickname || null });
    for (const key of Object.keys(obj)) { if (!['__typename','__ref','extensions','headers'].includes(key)) results.push(...extractQAFromJSON(obj[key], depth + 1)); }
  }
  return results;
}
function stripTags(html) { return (html || '').replace(/<[^>]*>/g, '').replace(/&amp;/g,'&').replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/&quot;/g,'"').replace(/&#39;/g,"'").replace(/&nbsp;/g,' ').replace(/\s+/g,' ').trim(); }
function decodeStr(str) { return (str||'').replace(/\\u([0-9a-fA-F]{4})/g,(_,h)=>String.fromCharCode(parseInt(h,16))).replace(/\\n/g,' ').replace(/\\"/g,'"').trim(); }
