...
async function searchKB(questionText, topK = 5) {
  try {
    ...
    const { data, error } = await supabase.rpc('search_kb_vector', { query_embedding: embedding, match_count: topK });
  ...
  try {
    ...
    if (!keywords) return [];
    const { data } = await supabase.from('knowledge_base').select('content,category,title').textSearch('content', keywords).limit(topK);
    return data || [];
  } catch (err) {
...
async function answerQuestion(question) {
  ...
  return generateRagAnswer(question.question_text, kbContext, productInfo);
}

module.exports = { searchKB, answerQuestion };
// BUILD: v2.5.20260628190134
